...
optimizing the model to predict the question, "Where was...".(important!我们只想要答案)
...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if length<len(document):
            tr_start=int(random.uniform(0,len(document)-length-1))
            tr_document=document[tr_start:tr_start+length]
        else:
            tr_start=int(random.uniform(0,len(document)-5))
            tr_document=document[tr_start:tr_start+length]

        masked_content_length=int(random.uniform(1,0.5*len(tr_document)-1))
        masked_content_start=int(random.uniform(1,len(tr_document)-masked_content_length-1))
        prefix=tr_document[0:masked_content_start]
        masked_content=tr_document[masked_content_start:masked_content_start+masked_content_length]
        suffix=tr_document[masked_content_start+masked_content_length:]

        pads=""
        for i in range(self.block_size-len(tr_document)-2):
            pads=pads+self.PAD_CHAR
